...
| `股票` | `wiki/股票/` | 个股档案、产业链位置、催化/基本面/消息面、验证框架、当前理解；不承载个人交易执行流水 |
...
> 股票页只记录公司研究和可复用理解；不要记录个人买入、卖出、成交价、数量、仓位、持仓流水、盈亏或交割单逐笔记录。
...
4. 将个人交易执行数据保留在 `raw/日复盘/`、`raw/交割单/` 等原始证据中；复盘结论进入 `总结/`、`策略/`、`错误/`、`模式/` 或 `data/brain/`。
5. 只有当资料提供独立的公司、产业链、催化、基本面、消息面或验证框架新信息时，才更新 `股票/股票名.md`，且不复制买卖执行细节。
6. 将策略迭代写入 `策略/策略名.md`。
7. 将市场结构和题材生命周期写入 `模式/`。
8. 将产业链、方法论和关键变量写入 `概念/`。
9. 将错误和反思写入 `错误/`。
10. 更新 `index.md`、`overview.md` 和 `wiki/logs/log-YYYY-MM-DD.md`。
11. 对会变化、会失效、会被后续来源替代的事实，写入 `data/facts/temporal_edges.jsonl`。
...
| 个股档案检查 | 找陈旧页面、重复股票页、误把交易执行流水写入股票页的情况 |
...
| 个人交易执行数据 | `raw/日复盘/`、`raw/交割单/`、`总结/`、`策略/`、`错误/`、`data/brain/` | 股票页只链接相关复盘，不复制买入/卖出/成交价/仓位/盈亏流水 |
| 股票研究档案 | `股票/股票名.md` | 只写公司、产业链、催化、基本面、消息面、验证框架和风险 |
